...
def _generate_parameters_for_analysis_check(algorithm: str,
    ...
    if technicals:
        return [algorithm, sample_IDs, nn_method, jaccard, resolution]

    return [algorithm, sample_IDs, gates, nn_method, jaccard, resolution]
...
def _score_gating(dataset: AnnData,
    ...
    for sample in gating["sample_ID"].unique():
        ...
        for flowjo_gate in list(gate_mapping.keys()):
            ...
            write_to_scores(f"phenograph,{sample},{flowjo_gate},{nn_method},{jaccard},{resolution},{score_string}",
                            output_dir = output_dir,
                            key = score_key)
    return

def _run_phenograph_benchmark(dataset: AnnData,
                              output_dir: str,
    ...
    resource_metrics = "algorithm,sample_ID,gate,nnmethod,jaccard,resolution," + scores + "\n"
    score_key = f"Scores"
    if continue_analysis is False or not os.path.isfile(os.path.join(output_dir, f"{score_key}.log")):
        write_to_scores(resource_metrics,
                        ...
                        key = score_key,
                        init = True)
    resolutions = [0.1, 0.5, 1, 5, 10, 15, 20]
    jaccards = [True, False]
    nn_methods = ["kdtree"]
    for resolution in resolutions:
        for jaccard in jaccards:
            for nn_method in nn_methods:
                parameters = _generate_parameters_for_analysis_check(algorithm = "phenograph",
    ...
    resource_metrics = "algorithm,sample_ID,nnmethod,jaccard,resolution,train_time,min_mem,max_mem,mean_mem\n"
            ...
            for nn_method in nn_methods:
                ...
                for sample in samples:
                    ...
                    mem_usage = memory_usage((clf.identify_populations, (), {"cluster_kwargs": {"resolution_parameter": resolution,
                                                                                                "jaccard": jaccard,
                                                                                                "nn_method": nn_method}}), interval = 10)
                    stop = time.time() - start
                    metric_string = f"phenograph,{sample},{nn_method},{jaccard},{resolution},{stop},{np.min(mem_usage)},{np.mean(mem_usage)},{np.max(mem_usage)}\n"
