...
def _generate_figure_a(fig: Figure,
    ...
    ax.axis("off")
    _figure_label(ax, subfigure_label, x = -0.47)
...
def _generate_figure_b(fig: Figure,
    ...
    clf_data = clf_data[clf_data["sampling"] == False]
...
def _generate_figure_c(fig: Figure,
    ...
    if dataset_name == "ZPM":
        ...
        plots_per_row = 4
    fig_sgs = gs.subgridspec(2,plots_per_row)
    col_coord = -1
    n_plots = 8 if dataset_name != "ZPM" else 6
    for i, gate in enumerate(gates_to_use[:n_plots]):
        df = _extract_fops(orig_gf, pred_gf, gate.split("/")[-1], dataset)
        row_coord = i%plots_per_row
        if row_coord == 0:
            col_coord += 1
...
def generate_supervised_characterization(dataset_name: Literal[
    ...
    last_col_height = 0.55 if dataset_name != "ZPM" else 0.7
    gs = GridSpec(ncols = 6,
                  nrows = 3,
                  figure = fig,
                  height_ratios = [0.8, 1, last_col_height])
